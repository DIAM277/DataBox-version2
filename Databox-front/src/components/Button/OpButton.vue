<template>
    <div v-if="showOp && fileData.fileId && fileData.status == 2 && !fileData.showEdit"
        ...
        <!-- 新增：动态渲染图标与高亮无缝切换 -->
        <el-tooltip :content="fileData.isFavorite === 1 ? '取消收藏' : '加入收藏'" placement="top" effect="dark"
            :show-after="300">
            <span
                class="w-7 h-7 flex items-center justify-center rounded-md hover:bg-white dark:hover:bg-gray-700 hover:shadow-sm transition-all cursor-pointer hidden md:flex"
                :class="fileData.isFavorite === 1 ? 'text-yellow-500 dark:text-yellow-400' : 'text-gray-400 dark:text-gray-500 hover:text-yellow-500 dark:hover:text-yellow-400'"
                @click.stop="handleFavorite">

                <!-- 实心星星 (已收藏) -->
                <svg v-if="fileData.isFavorite === 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                    fill="currentColor" class="w-[18px] h-[18px] transition-transform active:scale-90">
                ...
                <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                    stroke="currentColor" class="w-[18px] h-[18px] transition-transform active:scale-90">
